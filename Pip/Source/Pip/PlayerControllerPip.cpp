...
#include "PlayerControllerPip.h"

void APlayerControllerPipBase::OnPossess(APawn* aPawn)
{
	// Call the parent method, to let it do anything it needs to
	Super::OnPossess(aPawn);

	// Store a reference to the Player's Pawn
	PlayerCharacter = Cast<ACharacterPip>(aPawn);
	...
	if (ActionMove)
		EnhancedInputComponent->BindAction(ActionMove, ETriggerEvent::Triggered, this,
			&APlayerControllerPipBase::HandleMove);

	if (ActionLook)
		EnhancedInputComponent->BindAction(ActionLook, ETriggerEvent::Triggered, this,
			&APlayerControllerPipBase::HandleLook);
}

void APlayerControllerPipBase::OnUnPossess()
{
	// Unbind things here...
...
void APlayerControllerPipBase::HandleLook(const FInputActionValue& InputActionValue)
{
	// Input is a Vector2D
...
void APlayerControllerPipBase::HandleMove(const FInputActionValue& InputActionValue)
{
	// Value is a Vector2D
